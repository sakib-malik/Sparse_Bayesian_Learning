...
s2_act = 10.^(s2_dB/10); % list of noise variance in actual units [noise(actual) = 10^(noise(DB)/10)]
...
for sigma2 = s2_act  % for each of the noise variance
    ...
    % SBL takes as input [target (t), PHI (design matrix), N, M, beta(1 / sigma^2)]
    w_map(: , i) = SBL_ogl(t(:, i), Phi, N, M, 1/sigma2); % sparse parameter w_map for this noise variance
    
    i = i + 1;
end
fprintf("The Map estimate for w is\n");
disp(w_map);
%% Calculating AVG NMSE for each noise variance, averaged over 100 loops and plotting
...
for j = 1:100 % averaging over 100 times
    ...
    for sigma2 = s2_act
        ...
        w_map(: , i) = SBL_ogl(t(:, i), Phi, N, M, 1 / sigma2); % sparse parameter
